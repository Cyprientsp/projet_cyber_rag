...
\begin{titlepage}
  ...
  % --- Auteur / date ---
  {\large\bfseries\color{titleblue}Cyprien TCharlaté.\par}% <-- a personnaliser
  \vspace{4pt}
  {\color{codecmt}\today}
...
SAS (onduleurs photovoltaiques), dont les documents internes sous forment de markdown servent de base de connaissance.
...
chargement initial du modèle \texttt{gemma2:2b} en mémoire GPU (VRAM). Une fois le modèle
résident, les requêtes suivantes s'exécutent en \texttt{$\sim$0,6 s} (voir lignes 2 et 4).
\normalsize
\newpage
\section{Conclusion}
